  ...
  import { PAYROLL_QUERY } from '../sanity/queries'

  const NewPayroll = () => {
    // Fetch Payroll page data from Sanity
    const { data: payrollData } = useSanityData(PAYROLL_QUERY)
    ...
    const heroData = payrollData?.heroSection || {
      heading: 'Payroll Services',
      subheading: 'Every Employee Paid Right. Every Obligation Met.',
      backgroundImage: 'https://ik.imagekit.io/qxfudjvlf/taxarc/Payroll%20Services.jpeg',
    }

    // Who We Serve section with fallback
    const whoWeServeData = payrollData?.whoWeServeSection || {
    ...
    const servicesData = payrollData?.servicesSection || {
      heading: 'Our Payroll Services',
    ...
    const benefitsData = payrollData?.benefitsSection || {
